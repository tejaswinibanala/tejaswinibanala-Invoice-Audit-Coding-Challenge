...
import { InvoiceDrug, ReferenceDrug } from '../types';
import { 
  createMatchingDrugPair, 
  createFormulationDiscrepancyPair, 
  createStrengthDiscrepancyPair, 
  createPayerDiscrepancyPair,
  createTestDataArrays,
  createEdgeCaseData
} from './testDataFactory';

describe('Validation Service', () => {
  const { referenceDrugs: mockReferenceDrugs, invoiceDrugs: mockInvoiceDrugs } = createTestDataArrays({
    referenceCount: 3,
    invoiceCount: 3,
    discrepancyCount: 2
  });
  ...
  describe('Price Discrepancy Validation', () => {
    it('should flag price discrepancy when overcharge > 10%', async () => {
      const { reference, invoice } = createMatchingDrugPair(25); 
      
      const result = await validateDiscrepanciesService([invoice], [reference]);
    ...
    it('should not flag price discrepancy when overcharge <= 10%', async () => {
      ...
      const { reference, invoice } = createMatchingDrugPair(5); 
    ...
    it('should calculate correct overcharge amount', async () => {
      ...
      const { reference, invoice } = createMatchingDrugPair(14.29); // 14.29% overcharge
  ...
  describe('Formulation Discrepancy Validation', () => {
    it('should flag formulation mismatch ignoring packaging details', async () => {
      const { reference, invoice } = createMatchingDrugPair(0); // No price discrepancy
    ...
    it('should flag formulation mismatch for different formulations', async () => {
      const { reference, invoice } = createFormulationDiscrepancyPair();
  ...
  describe('Payer Discrepancy Validation', () => {
    ...
    it('should flag payer mismatch for different payers', async () => {
      const { reference, invoice } = createPayerDiscrepancyPair();
  ...
  describe('Summary Calculations', () => {
    it('should calculate correct summary totals', async () => {
      ...
      const { reference, invoice } = createMatchingDrugPair(25); // Price discrepancy
    ...
    it('should handle drugs not found in reference data', async () => {
      ...
      const result = await validateDiscrepanciesService(invoiceDrugs, mockReferenceDrugs);
  ...
  describe('Dynamic Data Testing', () => {
    it('should handle various drug data formats', async () => {
      ...
      const { referenceDrugs, invoiceDrugs } = createTestDataArrays({
        referenceCount: 5,
        invoiceCount: 5,
        discrepancyCount: 3
      });

      const result = await validateDiscrepanciesService(invoiceDrugs, referenceDrugs);
    ...
    it('should handle edge case data', async () => {
      ...
      const result1 = await validateDiscrepanciesService([edgeCases.largePrice.invoice], [edgeCases.largePrice.reference]);
      expect(result1.summary.totalIssues).toBeGreaterThan(0);
      
      // Test with very small numbers
      const result2 = await validateDiscrepanciesService([edgeCases.smallPrice.invoice], [edgeCases.smallPrice.reference]);
      expect(result2.summary.totalIssues).toBeGreaterThan(0);
      
      // Test with zero values
      const result3 = await validateDiscrepanciesService([edgeCases.zeroPrice.invoice], [edgeCases.zeroPrice.reference]);
